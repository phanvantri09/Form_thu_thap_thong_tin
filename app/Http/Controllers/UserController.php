...
class UserController extends Controller
{
    ...
    public function postpasss(Request $request){
        ...
        if(!empty($user[0]->id)){
            ...
            return redirect()->route('newpass', $email)->with('error',"Lỗi");
        }
    }
    public function getemail(Request $request){
        ...
        if(!empty($user[0]->id)){
            return redirect()->route('newpass', $email);
        }
        {
            return redirect()->route('changepass')->with('error',"Không có email nào như vậy");
        }
    }
    ...
    public function PostLogin(UserLogin $request){
        ...
        if (Auth::attempt($dataUser, $rememberMe))
        {
            ...
            return redirect()->route('trangchu')->with('success','Successfully Register, You can login!');
